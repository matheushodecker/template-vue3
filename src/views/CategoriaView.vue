...
.container {
  max-width: 900px; /* Aumentado levemente para mais espaço */
  margin: 40px auto;
  ...
  font-family: 'Inter', 'Helvetica Neue', Helvetica, Arial, sans-serif; /* Fonte moderna */
}
...
.form-group {
    ...
    gap: 15px; /* Mais espaço entre campos */
    margin-bottom: 20px;
}

input[type='text'], textarea {
    padding: 14px; /* Maior altura do campo */
...
input[type='text']:focus, textarea:focus {
    ...
    outline: none;
    box-shadow: 0 0 0 3px rgba(65, 184, 131, 0.2); /* Sutil brilho de foco */
}
...
button {
    padding: 12px 25px; /* Maior e mais fácil de clicar */
    border: none;
    border-radius: 6px;
    cursor: pointer;
...
button:hover:not(:disabled) {
    background-color: #358a66; /* Tom mais escuro */
...
.categoria-list {
  list-style: none;
  padding: 0;
  display: grid; /* Usando Grid para melhor espaçamento, se quiser pode ser flex */
  gap: 10px;
}
...
.categoria-list li {
  ...
  background-color: var(--white);
  border: 1px solid var(--border-color);
  border-left: 6px solid var(--accent-color); /* Borda mais grossa */
  border-radius: 8px;
...
.categoria-list li.categoria-inativa {
    ...
    opacity: 0.7; /* Sutilmente mais esmaecido */
}
...
.id-tag {
    ...
    padding: 4px 8px; /* Maior e mais legível */
    border-radius: 4px;
    font-size: 0.85rem;
    margin-right: 15px;
    color: #7f8c8d;
...
.descricao {
    ...
    white-space: nowrap;
    max-width: 400px; /* Limite o tamanho da descrição na lista */
}
...
.actions button {
    ...
    padding: 8px 15px;
    font-size: 0.95rem;
    text-transform: none; /* Não precisa de uppercase nas ações */
}
